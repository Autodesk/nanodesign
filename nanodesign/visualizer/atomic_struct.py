...
"""This module is used to visualize the atomic structure of a single strand (ssDNA) of a DNA model. 
   ...
   An atomistic model of a DNA nanodesign consists of one or more scaffold
   strands and many staple strands. Each strand of DNA is represnted as a
   polymer chain with a unique chain ID. The VisAtomicStructure class in this
   module creates and manages the visualization of a single strand. A DNA strand
   is visualized using several representations:

      bonds - The atomic bonds are displayed as lines. The sugar and base planes
              are displayed as filled polygons.
      backbone P atoms - The DNA P atoms are displayed as spheres connected by
                         lines.
