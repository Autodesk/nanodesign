...
try:
    ...
    # Define the location of the dna template PDB files for creating atomic structures.
    import nanodesign_transition
    dna_pdb_templates_dir = os.path.join(os.path.dirname(nanodesign_transition.__file__), 'converters')
    sys.path = sys.path[:-1]
except ImportError:
...
class AtomicStructure(object):
    ...
    def _read_template(self, infile_name):
        ...
        pdb_reader.read(file_name)
        molecules = pdb_reader.molecules
