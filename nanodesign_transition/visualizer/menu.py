...
class VisMenu(object):
    ...
    def _create_menu_items(self):
        ...
        self.selections[VisMenuEnity.MODEL] = model_select

        # Create helix sub-menus.
        helix_select = VisRepMenu(self, VisHelixRepType, self.helix_names, self.command.generate_helix_cmd)
        self.selections[VisMenuEnity.HELIX] = helix_select

        # Create strand sub-menus.
        strand_select = VisRepMenu(self, VisStrandRepType, self.strand_names, self.command.generate_strand_cmd)
        self.selections[VisMenuEnity.STRAND] = strand_select

        # Create atomic stucture sub-menus.
        if len(self.atomic_struct_names) > 2:
            atomic_select = VisRepMenu(self, atomic_struct.VisAtomicStructureRepType, self.atomic_struct_names, 
                self.command.generate_atomic_struct_cmd)
            self.selections[VisMenuEnity.ATOMIC_STRUCTURE] = atomic_select
        else:
            self.selections[VisMenuEnity.ATOMIC_STRUCTURE] = None 
            atomic_select = None 
        ...
        if atomic_select: 
            atomic_select.add_submenu(atomic_struct.VisAtomicStructureRepType.BACKBONE, "Atomic structure backbone") 
            atomic_select.add_submenu(atomic_struct.VisAtomicStructureRepType.BONDS,    "Atomic structure bonds") 
            atomic_select.add_submenu(atomic_struct.VisAtomicStructureRepType.CHECK,    "Atomic structure check") 
    ...
    def update_submenu_selection(self, type, name, rep, delay=False):
        """ Update a selection for the given rep and entity name. 
            ...
            Arguments:
                type (String): The menu entity type taken from VisMenuEnity.
