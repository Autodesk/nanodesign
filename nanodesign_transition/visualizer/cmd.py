...
import logging
import os
from menu import VisMenuEnity
import model 

class VisCommandEnity:
...
class VisCommand(object):
    ...
    def __init__(self, model, cmd_file_name, commands):
        ...
        self.parsing_map = {  
            VisCommandEnity.ATOMIC_STRUCTURE : self.proc_atomic_struct_cmd, 
            VisCommandEnity.COMMENT : self.proc_comment, 
            VisCommandEnity.GRAPHICS : self.proc_graphics_cmd, 
            VisCommandEnity.HELIX : self.proc_helix_cmd, 
            VisCommandEnity.MODEL : self.proc_model_cmd, 
            VisCommandEnity.STRAND : self.proc_strand_cmd 
        }
    ...
    def proc_helix_cmd(self, cmd, tokens):
        ...
        if (show and self.update_menu):
            delay = True
            self.model.menu.update_submenu_selection(VisMenuEnity.HELIX, name, rep, delay)
    ...
    def proc_model_cmd(self, cmd, tokens):
        ...
        if (show and self.update_menu):
            delay = True
            self.model.menu.update_selection(VisMenuEnity.MODEL, rep, delay)
    ...
    def proc_strand_cmd(self, cmd, tokens):
        ...
        if (show and self.update_menu):
            delay = True
            self.model.menu.update_submenu_selection(VisMenuEnity.STRAND, name, rep, delay)
    ...
    def proc_atomic_struct_cmd(self, cmd, tokens):
        ...
        if (show and self.update_menu):
            delay = True
            self.model.menu.update_submenu_selection(VisMenuEnity.ATOMIC_STRUCTURE, name, rep, delay)
